    ...
    def save_split(train, val, test, filepath, path_prefix):
        def _extract(items):
            ...
            for item in items:
                ...
                # Make paths relative to the dataset image directory. On Windows
                # os.path.join will treat paths starting with a slash or backslash
                # as absolute, so remove any leading slashes (both kinds).
                impath = impath.replace(path_prefix, "")
                # Strip both forward and backward slashes from the start
